...
if upload_protocol == "mbed":
    ...
    env.Replace(
        __configure_upload_port=__configure_upload_port,
        UPLOADER=join(
            '%s' % platform.get_package_dir("tool-airisp-next") or "",
            "AirISP-next"),
        UPLOADERFLAGS_1=[
            # "-c", board.get("upload.chip", "air001"),
            "-c auto",
            "-p"
        ],
        UPLOADERFLAGS_2=[
            "-b", env.subst("$UPLOAD_SPEED") or "115200", 
            "write_flash", 
            "-e", board.get("upload.offset_address", "0x08000000")
        ],
        UPLOADCMD='$UPLOADER $UPLOADERFLAGS_1 ${__configure_upload_port(__env__)} $UPLOADERFLAGS_2 $SOURCE'
        # UPLOADCMD='$UPLOADER $UPLOADERFLAGS "$SOURCE"'
    )

    #     # 定义一个打印UPLOADCMD的函数  
    # def print_upload_cmd(target, source, env):  
    #     # 展开UPLOADCMD模板  
    #     final_cmd = env.subst("$UPLOADCMD")  
    #     # 打印到控制台  
    #     print("UPLOAD COMMAND:", final_cmd)  
    #     return 0  # 返回0表示成功  

    upload_actions = [
        env.VerboseAction(env.AutodetectUploadPort, "Looking for upload port..."),
        # env.Action(print_upload_cmd, "Printing upload command..."),  # 添加这一行来打印命令
        env.VerboseAction("$UPLOADCMD", "Uploading $SOURCE")
